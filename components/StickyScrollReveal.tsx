  ...
  useEffect(() => {
    ...
    const sequence = async () => {
      while (isMounted) {
        ...
        // Step 4: 라인 그리기 2 (0.8초)
        if (isMounted) setStep(4);
        await new Promise(r => setTimeout(r, 800));
        
        // Step 5: 리포트 업데이트 (3초) - leads 숫자 증가
        if (isMounted) {
          setStep(5);
  ...
  return (
    <div className="w-full h-full relative overflow-hidden bg-gray-900/50">
        ...
        <div className="absolute bottom-12 left-12 w-48 h-48">
             ...
             <div className={`absolute top-10 right-10 w-3 h-3 bg-pink-500 rounded-full transition-all duration-300 ${step >= 1 ? 'opacity-100 scale-100 animate-ping' : 'opacity-0 scale-0'}`} />
             <div className={`absolute top-10 right-10 w-3 h-3 bg-pink-500 rounded-full shadow-[0_0_15px_#ec4899] transition-all duration-300 ${step >= 1 ? 'opacity-100 scale-100' : 'opacity-0 scale-0'}`} />
        </div>

        <div className={`absolute top-16 left-16 z-20 transition-all duration-500 ease-out ${step >= 3 ? 'opacity-100 translate-x-0 scale-100' : 'opacity-0 -translate-x-4 scale-95'}`}>
           <div className="bg-gray-800 border border-pink-500/50 px-4 py-2 rounded-xl shadow-[0_0_30px_rgba(236,72,153,0.3)] flex items-center gap-3">
              ...
              <div className="flex flex-col">
                 <span className="text-[10px] text-pink-400 font-bold tracking-widest">ALERT</span>
                 <span className="text-sm font-bold text-white whitespace-nowrap">New Lead +12%</span>
              </div>
           </div>
           
           <div className={`absolute -left-1.5 top-1/2 -translate-y-1/2 w-3 h-3 border-2 border-pink-500 bg-gray-900 rounded-full transition-colors duration-300 ${step >= 3 ? 'bg-pink-500' : 'bg-gray-900'}`} />
           <div className="absolute -right-1.5 top-1/2 -translate-y-1/2 w-2 h-2 bg-pink-500 rounded-full" />
        </div>

        <div className={`absolute bottom-12 right-12 w-48 bg-gray-800 border transition-all duration-300 rounded-xl p-4 shadow-2xl z-20 ${step === 5 ? 'border-pink-400 scale-105 shadow-[0_0_30px_rgba(236,72,153,0.4)]' : 'border-gray-700'}`}>
            <div className="flex justify-between items-center border-b border-gray-700/50 pb-2 mb-2">
               <span className="text-[10px] text-gray-400 font-bold uppercase">Campaign Report</span>
               <div className={`w-1.5 h-1.5 rounded-full ${step === 5 ? 'bg-pink-500 shadow-[0_0_8px_#ec4899]' : 'bg-gray-600'}`}></div>
            </div>
            <div className="space-y-1">
               <div className="text-xs text-gray-500">Total Leads</div>
               <div className={`text-3xl font-bold text-white tabular-nums transition-colors duration-300 ${step === 5 ? 'text-pink-300' : 'text-white'}`}>
                   {leads.toLocaleString()}
               </div>
            </div>
            
            <div className={`absolute -left-1.5 top-1/2 -translate-y-1/2 w-3 h-3 border-2 rounded-full transition-all duration-300 ${step >= 5 ? 'bg-pink-500 border-pink-500 shadow-[0_0_10px_#ec4899]' : 'bg-gray-800 border-gray-600'}`} />
        </div>
        ...
        <div className="absolute inset-0 pointer-events-none z-10">
           <svg className="w-full h-full overflow-visible">
              ...
              {step >= 2 && (
                <>
                  <path 
                    d="M 194 206 C 130 206, 64 160, 64 86" 
                    fill="none" 
                    stroke="url(#lineGrad)" 
                    strokeWidth="2" 
                    strokeDasharray="300" 
                  ...
                  />
                  <circle r="3" fill="#ec4899" className="animate-[fly-data_0.8s_ease-out_forwards]">
                    <animateMotion dur="0.8s" repeatCount="1" path="M 194 206 C 130 206, 64 160, 64 86" fill="freeze" />
                  </circle>
                </>
              )}
              ...
              {step >= 4 && (
                <>
                  <path 
                    d="M 230 86 C 280 86, 260 307, 310 307" 
                    fill="none" 
                    stroke="url(#lineGrad)" 
                    strokeWidth="2" 
                    strokeDasharray="300" 
                    ...
                    className="animate-draw-fast"
                  />
                  <circle r="3" fill="#fff" className="animate-[fly-data_0.8s_ease-in-out_forwards] shadow-[0_0_10px_white]">
                    <animateMotion dur="0.8s" repeatCount="1" path="M 230 86 C 280 86, 260 307, 310 307" fill="freeze" />
                  </circle>
                </>
              )}
           </svg>
    ...
    <div className="relative group">
       ...
       <div className="relative w-56 h-72 bg-gray-900 rounded-xl border border-gray-800 shadow-2xl flex flex-col overflow-hidden z-10">
          ...
          <div className="flex-1 p-5 space-y-4 relative bg-gray-900">
             {[...Array(6)].map((_, i) => (
                <div key={i} className="flex gap-2">
                   <div className="w-full h-2 rounded bg-gray-800 overflow-hidden relative">
                      <div className={`absolute inset-0 bg-amber-500/80 transform -translate-x-full animate-[flow_2s_linear_infinite]`} style={{ animationDelay: `${i * 0.15}s` }} />
                   </div>
            ...
            <div className="absolute top-[20%] left-[8%] bg-gray-800 border border-amber-500/30 p-2.5 rounded-md shadow-lg animate-[pop-data-wait_4s_linear_infinite]" style={{ animationDelay: '0.3s' }}>
            ...
            <div className="absolute top-[28%] right-[8%] bg-gray-800 border border-amber-500/30 p-2.5 rounded-md shadow-lg animate-[pop-data-wait_4s_linear_infinite]" style={{ animationDelay: '0.5s' }}>
               <div className="flex items-center gap-2">
                  <div className="w-2 h-2 rounded-full bg-amber-500" />
          ...
          <div className="absolute bottom-4 left-0 right-0 flex justify-center">
             <div className={`px-3 py-1 bg-gray-800 border rounded-full text-[10px] font-mono flex items-center gap-2 shadow-lg transition-colors duration-300 ${isComplete ? 'border-green-500/40 text-green-400' : 'border-amber-500/20 text-amber-500'}`}>
                <span className="relative flex h-2 w-2">
                  <span className={`animate-ping absolute inline-flex h-full w-full rounded-full opacity-75 ${isComplete ? 'bg-green-400' : 'bg-amber-400'}`}></span>
                  <span className={`relative inline-flex rounded-full h-2 w-2 ${isComplete ? 'bg-green-500' : 'bg-amber-500'}`}></span>
                </span>
                Extracted: {percent}%
             </div>
          </div>
  ...
  return (
    <div className="w-72 bg-gray-900 border border-gray-800 rounded-xl p-6 shadow-2xl overflow-hidden relative select-none">
       ...
       <div className="flex justify-between items-baseline mb-8 relative z-10">
          ...
          <span className={`text-3xl font-bold transition-all duration-300 ${showResult ? 'text-lime-400 scale-125' : 'text-gray-500 scale-100'}`}>
             A+
          </span>
       </div>
       ...
       <div className="space-y-6 relative z-10">
          <div className="space-y-3">
             <div className="flex justify-between text-xs items-center">
                <span className="text-gray-300 font-semibold">Daily Budget Limit</span>
                <span className={`font-mono transition-colors duration-300 ${isClicking ? 'text-lime-400' : 'text-gray-400'}`}>
             ...
             <div className="h-2 bg-gray-700 rounded-full w-full relative">
                <div 
                  className="absolute top-0 left-0 h-full bg-lime-500 rounded-full transition-all duration-75 ease-linear"
                  style={{ width: `${sliderWidth}%` }}
                />
                <div 
                  className={`absolute top-1/2 -translate-y-1/2 w-4 h-4 bg-white rounded-full shadow-lg transition-all duration-75 ease-linear ${isClicking ? 'scale-110 shadow-[0_0_10px_rgba(132,204,22,0.8)]' : 'scale-100'}`}
                  style={{ left: `${sliderWidth}%`, transform: `translate(-50%, -50%) ${isClicking ? 'scale(1.2)' : 'scale(1)'}` }}
                />
             </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2 mt-4">
             ...
             <div className={`bg-gray-800/50 p-2 rounded border transition-colors duration-500 ${showResult ? 'border-lime-500/50 bg-lime-900/20' : 'border-gray-700'}`}>
                <div className="text-[10px] text-gray-500">ROI</div>
                <div className={`text-sm font-bold ${showResult ? 'text-lime-400' : 'text-white'}`}>340%</div>
             </div>
          </div>
       </div>
    </div>
  ...
  return (
    <div className="w-72 h-64 bg-gray-900 border border-gray-800 rounded-xl relative overflow-hidden flex items-center justify-center">
       ...
       <svg className="absolute inset-0 w-full h-full p-4 overflow-visible">
         ...
         <line x1="0" y1="120" x2="100%" y2="120" stroke="#ef4444" strokeWidth="1" strokeDasharray="4 4" opacity="0.5" />
       ...
       <div className="absolute z-10 flex flex-col items-center top-1/2 -translate-y-1/2">
          ...
          <div className={`transition-all duration-300 transform ${showWarning ? 'opacity-100 translate-y-0 scale-100' : 'opacity-0 -translate-y-2 scale-95'} px-3 py-1 bg-red-950 border border-red-500/30 rounded text-[10px] text-red-400 font-mono tracking-wider flex items-center gap-2 shadow-lg`}>
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        ...
        <div className="mb-10 text-center">
          <h2 className="text-3xl md:text-5xl font-bold text-white mb-6">
            사업자는 다시 <span className="text-purple-500">'사업'</span>에 집중할 수 있습니다
          </h2>
        ...
        <div className="hidden lg:flex gap-10">
                ...
                {FEATURES.map((feature, index) => (
                  <div
                    ...
                    className={`absolute inset-0 flex flex-col items-center justify-center p-8 transition-all duration-500 ease-in-out ${
                      activeCard === index ? 'opacity-100 translate-y-0 scale-100' : 'opacity-0 translate-y-8 scale-95 pointer-events-none'
                    }`}
                  >
                ...
                {FEATURES.map((feature, index) => {
                  ...
                  const getColorStyle = () => {
                    ...
                    const colorMap: { [key: string]: string } = {
                      'from-amber-500 to-orange-600': 'linear-gradient(to right, rgb(245, 158, 11), rgb(234, 88, 12))',
                      'from-emerald-500 to-cyan-600': 'linear-gradient(to right, rgb(16, 185, 129), rgb(8, 145, 178))',
                      'from-pink-600 to-rose-600': 'linear-gradient(to right, rgb(219, 39, 119), rgb(225, 29, 72))',
                      'from-lime-400 to-green-600': 'linear-gradient(to right, rgb(163, 230, 53), rgb(22, 163, 74))',
                      'from-red-600 to-orange-600': 'linear-gradient(to right, rgb(220, 38, 38), rgb(234, 88, 12))',
                    };
                    return { background: colorMap[feature.color] || `linear-gradient(to right, ${feature.color})` };
                  };
          ...
          <div className="w-1/2 flex flex-col">
            {FEATURES.map((feature, index) => (
              ...
              >
                <div className={`transition-all duration-500 ${activeCard === index ? 'opacity-100 translate-x-0' : 'opacity-40 translate-x-10'}`}>
                  <h3 className="text-4xl font-bold text-white mb-4">
                    {feature.title} <span className={`bg-clip-text text-transparent bg-gradient-to-r ${feature.color} font-extrabold`}>{feature.highlight}</span>
                  </h3>
        ...
        <div className="lg:hidden flex flex-col gap-12">
          {FEATURES.map((feature, index) => (
            <div key={index} className="bg-gray-900 border border-gray-800 rounded-2xl p-8 overflow-hidden relative">
              ...
              <div className={`text-2xl font-bold bg-clip-text text-transparent bg-gradient-to-r ${feature.color} mb-4`}>
                {feature.highlight}
              </div>
