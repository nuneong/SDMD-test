  ...
  return (
    <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-0">
      ...
      >
        <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-slate-900 p-2 rounded-full border border-slate-700 shadow-xl">
          <div className="relative w-6 h-6">
            <Sun
              className="absolute w-6 h-6 text-amber-400 opacity-0 animate-[pulse_2s_infinite]"
              style={{ opacity: isDay ? 1 : 0, transition: "opacity 1s" }}
            />
            <Moon
              className="absolute w-6 h-6 text-indigo-300 opacity-0"
              style={{ opacity: !isDay ? 1 : 0, transition: "opacity 1s" }}
            />
          </div>
        </div>
      ...
      <div className="absolute -bottom-[48px] right-10 translate-x-[110px] -translate-y-[20.5px] flex flex-col items-center gap-2">
        <span className="text-xs text-slate-400 font-medium translate-y-[2px]">
          REAL-TIME OPS
        </span>
        <div className="flex items-center gap-2">
          <Clock className="w-4 h-4 text-purple-400 animate-pulse" />
          <span className="font-mono text-xl text-purple-400 font-bold">
            {formattedTime}
          </span>
        </div>
      </div>
